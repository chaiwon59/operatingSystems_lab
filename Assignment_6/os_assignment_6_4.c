...
static int os_mkdir(const char* path, mode_t mode) {
    ...
    //Takes a path string and returns all but the last part,
    //a path towards what is on the path
    char *dirs = get_dirs_from_path(path);

    //Takes a path string and returns the very last part, the name
    char *name = get_name_from_path(path);

    //Find a directory (by path)
    dir_t* dir = find_dir(root_dir, dirs);
    if(dir == NULL){
        return -ENOENT;
    }

    //Create directory with name
    dir_t* newDir = create_dir(name);

    //Adds a directory to another given directory
    add_dir_to(dir, newDir);
...
static int os_read(const char* path, char* buff, size_t size, off_t off, struct fuse_file_info* fi) {
    ...
    size_t length = strlen(file->contents);
    if(off > length) {
        return 0;
    }else if(length < (size + off)){
        size -= (length - off);
    }
...
static int os_write(const char* path, const char* buff, size_t size, off_t off, struct fuse_file_info* fi) {
    ...
    if((off + size) > length){
        ...
        char *newBuff = realloc(file->contents, (off + size + 1));
        if(newBuff != NULL){
            file->contents = newBuff;
        }else{
            return 0;
        }
    }

    //Copies 'size' characters from memory area buff to memory area file->contents
...
static int os_create(const char * path, mode_t mode, struct fuse_file_info* fi) {
    ...
    //Creates a file with a name (that we input) with empty contents
    file_t* newFile = create_file(name, "");

    //Add this new file to a given directory
    add_file_to(dir, newFile);
...
static int os_truncate(const char* path, off_t off) {
    ...
    size_t new_length = off + 1;
    file->contents = realloc(file->contents, new_length);

    if (new_length > cur_length) {
        memset(file->contents + cur_length, ' ', new_length - cur_length);
    }

    file->contents[new_length - 1] = 0;
